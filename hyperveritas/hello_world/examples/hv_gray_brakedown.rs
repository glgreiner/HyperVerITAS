     ...
     (impl (TranscriptWrite<<Pcs as PolynomialCommitmentScheme<F>>::CommitmentChunk, F>) + InMemoryTranscript)) 
{
    ...
    let grayFileName = format!("test/Gray{}.json", input_size);
...
fn verify(
    ...
    let fileName = format!("test/Gray{}.json", numCols);
...
fn main(){
    ...
    for i in first_size..last_size+1 {
        println!("-----------------------------------------------------------------------");
        println!("Full System Grayscale, HyperVerITAS Brakedown 127. Size: 2^{:?}\n", i);
