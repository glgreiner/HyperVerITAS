...
fn product_check(pp: <Pcs as PolynomialCommitmentScheme<F>>::ProverParam, fxs: MultilinearPolynomial<F>, gxs: MultilinearPolynomial<F>, 
    transcript: &mut impl TranscriptWrite<<Pcs as PolynomialCommitmentScheme<F>>::CommitmentChunk, F>, start: usize) -> (Expression<F>, Vec<MultilinearPolynomial<F>>, Vec<F>, Vec<F>, Vec<MultilinearBrakedownCommitment<F, Blake2s256>>) 
{
...
pub fn multsetCreatePolys(
    ...
    start: usize
) -> (Expression<F>, Vec<MultilinearPolynomial<F>>, Vec<F>, Vec<F>, Vec<MultilinearBrakedownCommitment<F, Blake2s256>>)
{
